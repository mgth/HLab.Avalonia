...
{
   public class Bootloader(IIconService icons) : Core.Annotations.Bootloader
   {
      protected override BootState Load()
      {
         if (icons is not IconService i) throw new InvalidCastException("IconService Bootloader should be used with IconService");
         ...
         foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies().Where(e => !e.IsDynamic))
         {
            ...
            foreach (var uri in uris)
            {
               ...
               {
                  i.AddIconProvider($"{prefix}/{k}", new IconProviderSvgFromUri(uri, Colors.Black));
               }
            }
         }
         i.ServiceState = ServiceState.Available;
         return base.Load();
      }
